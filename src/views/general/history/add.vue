<template>
	<el-card style="min-height: 100%;">
		<el-form :model="historyForm" :rules="rules" ref="historyRef" label-width="100px" style="margin-top: 30px;">
			<el-form-item label="内容" prop="content">
				<div style="border: 1px solid #ccc; z-index: 100">
					<Toolbar style="border-bottom: 1px solid #ccc" :editor="editorRef" :defaultConfig="toolbarConfig"
						:mode="mode" />
					<Editor style="height: 500px; overflow-y: hidden;" v-model="historyForm.content"
...
<script>
	...
	export default {
		...
		setup() {
			...
			onMounted(() => {
				if (id) {
					axios.get(`/api/back/historys/${id}`).then(res => {
						state.historyForm = {
			...
			const submitAdd = () => {
				historyRef.value.validate((vaild) => {
					if (vaild) {
						// 默认新增用 post 方法
						let httpOption = axios.post
						let params = {
							content: state.historyForm.content,
						}
			
						let url = '/api/back/historys'
						
						if (id) {
							// params.id = id
							// 修改商品使用 put 方法
							httpOption = axios.put
							url = `/api/back/historys/${id}`
						}
