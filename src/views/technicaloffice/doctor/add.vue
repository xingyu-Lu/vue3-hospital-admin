<template>
	<el-card style="min-height: 100%;">
		<el-form :model="Form" :rules="rules" ref="Ref" label-width="100px">
			...
			<el-form-item label="医生图片" prop="img">
	...
	export default {
		...
		setup() {
			...
			onMounted(() => {
				...
				if (id) {
					axios.get(`/api/back/technicalOfficeDoctors/${id}`).then(res => {
						state.Form = {
			...
			const submitAdd = () => {
				Ref.value.validate((vaild) => {
					if (vaild) {
						...
						let url = '/api/back/technicalOfficeDoctors'
						if (id) {
							// params.id = id
							// 修改商品使用 put 方法
							httpOption = axios.put
							url = `/api/back/technicalOfficeDoctors/${id}`
						}
