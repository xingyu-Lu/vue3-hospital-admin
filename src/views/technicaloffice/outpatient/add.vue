<template>
	<el-card style="min-height: 100%;">
		<el-form :model="briefForm" :rules="rules" ref="outpatientRef" label-width="100px" style="margin-top: 30px;">
			<el-form-item label="内容" prop="content">
				<div style="border: 1px solid #ccc; z-index: 100">
					<Toolbar style="border-bottom: 1px solid #ccc" :editor="editorRef" :defaultConfig="toolbarConfig"
						:mode="mode" />
					<Editor style="height: 500px; overflow-y: hidden;" v-model="briefForm.content"
...
<script>
	...
	export default {
		...
		setup() {
			...
			onMounted(() => {
				if (id) {
					axios.get(`/api/back/technicalOfficeOutpatientNews/${id}`).then(res => {
						state.briefForm = {
			...
			const submitAdd = () => {
				outpatientRef.value.validate((vaild) => {
					if (vaild) {
						// 默认新增用 post 方法
						let httpOption = axios.post
						let params = {
							content: state.briefForm.content,
						}
			
						let url = '/api/back/technicalOfficeOutpatientNews'
						
						if (id) {
							// params.id = id
							// 修改商品使用 put 方法
							httpOption = axios.put
							url = `/api/back/technicalOfficeOutpatientNews/${id}`
						}
