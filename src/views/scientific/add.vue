		...
		<el-form :model="Form" :rules="rules" ref="Ref" label-width="100px">
			<el-form-item label="类型" prop="type">
				<el-select v-model="Form.type" placeholder="Select" filterable>
					<el-option label="科研动态" value="0"></el-option>
					<el-option label="学科建设" value="1"></el-option>
					<el-option label="继续教育" value="2"></el-option>
				</el-select>
			</el-form-item>
			...
			const submitAdd = () => {
				Ref.value.validate((vaild) => {
					if (vaild) {
						...
						let url = '/api/back/scientifics'
						if (id) {
							// params.id = id
							// 修改商品使用 put 方法
							params.release_time = state.Form.release_time
							httpOption = axios.put
							url = `/api/back/scientifics/${id}`
						}
